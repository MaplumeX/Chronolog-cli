...
- Before stopping a timer, check the user's continuous-timing mode with `chronolog auth status` (`user.continuousTiming`). When it is on, `timer stop` does not fully stop: the server stops the current segment and immediately starts a new empty one. The returned entry with `stoppedAt: null` means a new segment is running, not that the stop failed. To stop timing completely, first run `chronolog account profile --continuous-timing false` (confirm this setting change with the user), then `chronolog timer stop`. To switch to a different task while timing, run `chronolog timer start` with the new category and description directly: the server stops the previous entry automatically, so do not stop-then-start. If a continuous-timing stop already produced an empty segment (no category, empty description — expected, not data loss), fill it in with `chronolog timer edit`, or merge it into an adjacent entry with `entries merge`.
